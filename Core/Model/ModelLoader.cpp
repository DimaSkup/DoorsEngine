...
//---------------------------------------------------------
// Desc:    read in all data from the .de3d file and fill in
//          the input model with this data;
// Args:    - path:      path to model relatively to the "assets" folder
// Output:  - outModel:  a model object filled with loaded data
//---------------------------------------------------------
bool ModelLoader::Load(const char* path, BasicModel& outModel)
{
    // check input args
    if (!path || path[0] == '\0')
    {
        LogErr(LOG, "input path to model is empty");
        return false;
    }


    // generate a full path to the model file
    char modelPath[256]{ '\0' };
    strcat(modelPath, g_RelPathAssetsDir);
    ...
    const char* modelDirPath)
{
    ...
    for (int i = 0; i < model.numSubsets_; ++i)
    {
        ...
        {
            // generate a full path to texture...
            memset(g_String, 0, LOG_BUF_SIZE);
            strcat(g_String, modelDirPath);
            strcat(g_String, "/textures/");
            strcat(g_String, params.texPaths[j].c_str());

            // ...and load it
            g_TextureMgr.LoadFromFile(g_String);
        }
    }
}
