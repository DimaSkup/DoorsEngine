...
//---------------------------------------------------------
// Desc:  helper for rotation a vector around Y-axis ;
//        if angle positive we rotate a vector
//        in a clockwise order when look down along Y-axis
//        (look in <0, -1, 0> direction)
//
...
//        it is 8 plane (one for each direction); 4 basic planes
//        are rotated by each other by 45 degrees;
//        8 planes because we use a unique image for each side
//        (numerated on visualization, imagine a plane's normal vector
//         from the center to some number) and don't use cull:none mode;
//
// Upside-down view (4 basic planes):       View on one plane (with indices order):
// 
//                 4                              1 *---------------* 2
//            3    |    5                           |             / |
//             \   |   /                            |           /   |
...
{
    constexpr int numVertices = 32;             // 8*4: 8 planes, 4 vertices per plane
    constexpr int numIndices  = 48;             // 8*6: 8 planes, 6 indices per plane
    ...
    const float texOffsetX = 0.125f;            // 1/8


    // setup vertices for all 8 planes
    const uint angleStep = 45;

    for (uint i = 0; i < 8; ++i)
    {
        const float angleX   = DEG_TO_RAD(rotateAroundX);
        const float angleY   = DEG_TO_RAD(i * angleStep);
    ...
    const UINT indices[48] =
    {
        ...
        12,13,14,   12,14,15,  // plane_3 (vertex[12-15])

        16,17,18,   16,18,19,  // plane_4 (vertex[16-19])
        20,21,22,   20,22,23,  // plane_5 (vertex[20-23])
        24,25,26,   24,26,27,  // plane_6 (vertex[24-27])
        28,29,30,   28,30,31,  // plane_7 (vertex[28-31])
    };
...
void GeometryGenerator::GenerateCube(BasicModel& model)
{
    ...
    vertices[1] = { pos4, tex[1], normFront, tangents[0] };
    vertices[2] = { pos0, tex[2], normFront, tangents[0] };
    vertices[3] = { pos1, tex[3], normFront, tangents[0] };

    // back
    vertices[4] = { pos3, tex[0], normBack, tangents[1] };
    vertices[5] = { pos2, tex[1], normBack, tangents[1] };
    vertices[6] = { pos6, tex[2], normBack, tangents[1] };
    vertices[7] = { pos7, tex[3], normBack, tangents[1] };

    // left
    vertices[8]  = { pos7, tex[0], normLeft, tangents[2] };
    vertices[9]  = { pos6, tex[1], normLeft, tangents[2] };
    vertices[10] = { pos4, tex[2], normLeft, tangents[2] };
    vertices[11] = { pos5, tex[3], normLeft, tangents[2] };

    // right
    vertices[12] = { pos1, tex[0], normRight, tangents[3] };
    vertices[13] = { pos0, tex[1], normRight, tangents[3] };
    vertices[14] = { pos2, tex[2], normRight, tangents[3] };
    vertices[15] = { pos3, tex[3], normRight, tangents[3] };

    // top
    vertices[16] = { pos4, tex[0], normTop, tangents[4] };
    vertices[17] = { pos6, tex[1], normTop, tangents[4] };
    vertices[18] = { pos2, tex[2], normTop, tangents[4] };
    vertices[19] = { pos0, tex[3], normTop, tangents[4] };

    // bottom
    vertices[20] = { pos7, tex[0], normBottom, tangents[5] };
    vertices[21] = { pos5, tex[1], normBottom, tangents[5] };
    vertices[22] = { pos1, tex[2], normBottom, tangents[5] };
...
//---------------------------------------------------------
// build indices for the sphere by input params
//---------------------------------------------------------
    ...
    const int stackCount)
{
    ...
    for (int i = 0; i < stackCount - 1; ++i)
    {
        for (int j = 0; j < sliceCount; ++j)
        {
            ...
            // first triangle of the sphere face
            outIndices[indexIdx + 0] = idx_1;
            outIndices[indexIdx + 1] = idx_2;
            outIndices[indexIdx + 2] = idx_3;

            // second triangle of the sphere face
            outIndices[indexIdx + 3] = idx_1;
            outIndices[indexIdx + 4] = idx_3;
            outIndices[indexIdx + 5] = idx_4;
    ...
    {
        outIndices[indexIdx + 0] = centerIdx;
        outIndices[indexIdx + 1] = i;
        outIndices[indexIdx + 2] = i + 1;
...
//---------------------------------------------------------
// compute an axis-aligned bounding box of the mesh
//---------------------------------------------------------
...
//---------------------------------------------------------
// Desc:   generate a curved plane for sky clouds
...
// Out:    - outVertices:        output array of generated vertices
//         - outIndices:         output array of generated indices
//         - outNumVertices:     output number of generated vertices
//         - outNumIndices:      output number of generated indices
//---------------------------------------------------------
bool GeometryGenerator::GenSkyPlane(
    ...
    int& outNumIndices)
{
    float quadSize = 0;   // the size of each quad on the plane
    float radius = 0;     // the radius of the sky plane based on the width
    float dh = 0;         // the height constant to increment by
    float texDelta = 0;   // the texture coordinate increment value


    // create the array to hold the sky plane coords
    const int numVertices = SQR(skyPlaneResolution + 1);

    VertexPosTex* tmpVertices = NEW VertexPosTex[numVertices];
    if (!(tmpVertices))
    {
        LogErr(LOG, "can't alloc memory for sky plane's vertices");
        return false;
    }

    quadSize = skyPlaneWidth / (float)skyPlaneResolution;
    radius   = skyPlaneWidth / 2.0f;
    dh       = (skyPlaneTop - skyPlaneBottom) / SQR(radius);
    texDelta = (float)texRepeat / (float)skyPlaneResolution;


    // loop through each vertex and calc its params
    for (int j = 0; j <= skyPlaneResolution; ++j)
    {
        for (int i = 0; i <= skyPlaneResolution; ++i)
        {
            // calc the index into the sky plane arr to add this vertex
            int idx = j * (skyPlaneResolution + 1) + i;
            VertexPosTex& v = tmpVertices[idx];

            float fi = (float)i;
            float fj = (float)j;

            v.pos.x = (-0.5f * skyPlaneWidth) + (fi * quadSize);
            v.pos.z = (-0.5f * skyPlaneWidth) + (fj * quadSize);
            v.pos.y = skyPlaneTop - (dh * (SQR(v.pos.x) + SQR(v.pos.z)));
    ...
    // --- prepare arrays which we will use to init vertex and index buffers --- //
    constexpr int numVerticesInQuad = 6;

    // calc number of vertices and indices in sky plane mesh
    outNumVertices = SQR(skyPlaneResolution + 1) * numVerticesInQuad;
    outNumIndices  = outNumVertices;

    // alloc arrays
    *outVertices = NEW VertexPosTex[outNumVertices];
    assert(*outVertices != nullptr);

    *outIndices  = NEW uint16[outNumIndices];
    assert(*outIndices != nullptr);


    // init final arrays
    int idx = 0;
    VertexPosTex* vertices = *outVertices;
    ...
    for (int j = 0; j < skyPlaneResolution; ++j)
    {
        for (int i = 0; i < skyPlaneResolution; ++i)
        {
            const int i1 = j * (skyPlaneResolution + 1) + i;
            const int i2 = j * (skyPlaneResolution + 1) + (i+1);
            const int i3 = (j+1) * (skyPlaneResolution + 1) + i;
            const int i4 = (j+1) * (skyPlaneResolution + 1) + (i+1);

            // triangle 1 - upper left
            vertices[idx] = tmpVertices[i1];
            indices[idx]  = idx;
            idx++;

            // triangle 1 - upper right
            vertices[idx] = tmpVertices[i2];
            indices[idx]  = idx;
            idx++;

            // triangle 1 - bottom left
            vertices[idx] = tmpVertices[i3];
            indices[idx]  = idx;
            idx++;
            ...
            vertices[idx] = tmpVertices[i4];
            indices[idx]  = idx;
            idx++;
        }
    }

    // release memory since we already calculated all what we need
    SafeDeleteArr(tmpVertices);

    return true;
}
//---------------------------------------------------------
// Desc:   setup the vertices' positions of a cube
//---------------------------------------------------------
...
//---------------------------------------------------------
// Desc:  setup the faces' normals of a cube
//---------------------------------------------------------
...
//---------------------------------------------------------
// Desc:  a cylinder is divided horizontally into stacks, so we compute them here
//---------------------------------------------------------
