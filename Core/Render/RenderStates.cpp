...
void RenderStates::SetBS(ID3D11DeviceContext* pDeviceContext, const STATES key)
{
	...
	try
	{
		ID3D11BlendState* pBS = blendStates_.at(key);
		...
		case NO_RENDER_TARGET_WRITES:
		{
			...
			pDeviceContext->OMSetBlendState(pBS, blendFactor, 0xFFFFFFFF);
			break;
		}
		default:
		{
	...
	{
		Log::Error(e.what());
		Log::Error("unknown key to a blend state: " + std::to_string(key));

		pDeviceContext->OMSetBlendState(nullptr, NULL, 0xFFFFFFFF);
	}
	...
	const UINT stencilRef)
{
	try
	{
		...
		pDeviceContext->OMSetDepthStencilState(depthStencilStates_.at(key), stencilRef);
	}
	catch (const std::out_of_range& e)
	{
		Log::Error(e.what());
		Log::Error("unknown key to a depth stencil state: " + std::to_string(key));
