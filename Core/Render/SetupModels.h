...
void SetupTree(BasicModel& tree)
{
	...
	const std::string pathToModelDir = g_ModelsDirPath + "trees/FBX format/";
...
void SetupPowerLine(BasicModel& powerLine)
{
	...
	TextureMgr& texMgr = *TextureMgr::Get();
	const std::string texDirPath = g_ModelsDirPath + "power_line/";
...
{
	// setup materials
	for (int i = 0; i < house.numMats_; ++i)
	{
...
void SetupStalkerAbandonedHouse(BasicModel& house)
{
	...
	const std::string dirPath = g_ModelsDirPath + "/stalker/abandoned-house-20/";
...
{
	TextureMgr& texMgr = *TextureMgr::Get();
	TexID texID = texMgr.LoadFromFile(g_TexDirPath + "gigachad.dds");
