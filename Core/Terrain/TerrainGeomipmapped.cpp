...
//---------------------------------------------------------
void TerrainGeomip::ComputeBoundings()
{
    ...
    for (int pz = 0; pz < numPatchesPerSide; ++pz)
    {
        for (int px = 0; px < numPatchesPerSide; ++px)
        {
            ...
            // visualize debug AABB of each terrain's patch
            Core::g_DebugDrawMgr.AddTerrainAABB(aabbMinPoint, aabbMaxPoint, colorYellow);
    ...
    const int numIndices)
{
    ...
    if (!vb_.Initialize(Render::g_pDevice, vertices, numVertices, isDynamicBuf))
    {
    ...
    if (!ib_.Initialize(Render::g_pDevice, indices, numIndices, isDynamicBuf))
    {
...
//---------------------------------------------------------
//---------------------------------------------------------
bool TerrainGeomip::IsPatchInsideViewFrustum(const int patchIdx, const Frustum& frustum)
{
...
void TerrainGeomip::PopulateBuffers()
{
    ...
    //memcpy(vertices_, vertices.data(), sizeof(Vertex3dTerrain) * vertices.size());
