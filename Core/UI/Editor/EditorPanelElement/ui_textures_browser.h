// =================================================================================
// Filename:    ui_textures_browser.h
...
public:
    ...
    inline float GetWndSizeX()      const { return iconSize_ * 25; }
...
private:
    ...
    void DrawTextureIcons (const float startPosX, const float startPosY);
    void ComputeZooming   (const float startPosX, const float startPosY, const float availWidth);
...
private:
    ...
    float   prevAvailWidth_  = 0;      // if curr and prev avail width aren't equal - we call UpdateLayoutSizes()
    float   iconSize_        = 96;
    float   zoomWheelAccum_  = 0;      // Mouse wheel accumulator to handle smooth wheels better
    int     iconSpacing_     = 8;
    ...
    float layoutItemSpacing_       = 0;
    float layoutSelectableSpacing_ = 0;
    float layoutOuterPadding_      = 0;
