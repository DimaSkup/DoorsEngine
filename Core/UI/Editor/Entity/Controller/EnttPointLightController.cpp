...
void EnttPointLightController::LoadEnttData(const EntityID id)
{
    ...
    if (!pFacade_->GetEnttPointLightData(
        ...
        model.isActive))
    {
        LogErr(LOG, "can't load point light component data of entt ID: %" PRIu32, id);
    }
}
...
void EnttPointLightController::ExecuteCommand(const ICommand* pCmd,	const EntityID id)
{
    switch (pCmd->type_)
    {
        case CHANGE_POINT_LIGHT_ACTIVATION:
            ExecChangeActivation(id, pCmd->GetFloat());
            break;
...
void EnttPointLightController::UndoCommand(const ICommand* pCmd, const EntityID id)
{
    ...
    switch (pCmd->type_)
    {
        ...
        default:
        {
            LogErr(LOG, "unknown undo command (point light) for entity: %" PRIu32, id);
        }
    }
}
...
void EnttPointLightController::ExecChangeActivation(const EntityID id, const float isActive)
{
    const float oldActivation = pFacade_->GetLightIsActive(id);

    if (pFacade_->SetLightActive(id, isActive))
    {
        // update editor fields
        pointLightModel_.isActive = (bool)isActive;
