...
public:
	...
	void Draw(Model::Sky* pData)
	{
		//
		// show the sky editor fields
		//
		...
		{
			pListener_->Execute(new CmdChangeColor(CHANGE_COLOR_CENTER, colorCenter));
		}

		if (ImGui::ColorEdit3("Apex color", colorApex.rgb_))
		{
			pListener_->Execute(new CmdChangeColor(CHANGE_COLOR_APEX, colorApex));
		}

		if (ImGui::DragFloat3("Sky offset", offset.xyz_))
		{
			pListener_->Execute(new CmdChangeVec3(CHANGE_SKY_OFFSET, offset));
		}
	}
};
