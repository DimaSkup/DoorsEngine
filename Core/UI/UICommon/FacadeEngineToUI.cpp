...
// static variables for internal purposes
    ...
    pTerrain_(pTerrain)
{
    ...
    CAssert::NotNullptr(pEngine,    "ptr to engine == null");
    CAssert::NotNullptr(pRender,    "ptr to render == null");
    CAssert::NotNullptr(pEntityMgr, "ptr to the entt mgr == null");
    CAssert::NotNullptr(pGraphics,  "ptr to the graphics class == null");
    CAssert::NotNullptr(pTerrain,   "ptr to the terrain == null");

    CAssert::True(eEnttComponentType::NUM_COMPONENTS == ECS::eComponentType::NUM_COMPONENTS, "both enums are not the same");
}
    ...
    float* outProj)
{
    if (!outView)
    {
        LogErr(LOG, "view matrix ptr == null");
        return;
    }
    if (!outProj)
    {
        LogErr(LOG, "proj matrix ptr == null");
        return;
    }

    const DirectX::XMMATRIX& view = pEnttMgr_->cameraSys_.GetView(camEnttID);
    const DirectX::XMMATRIX& proj = pEnttMgr_->cameraSys_.GetProj(camEnttID);
...
void FacadeEngineToUI::FocusCameraOnEntity(const EntityID enttID)
{
    ...
    DirectX::XMFLOAT3 enttPos = mgr.transformSys_.GetPositionByID(enttID);
...
//---------------------------------------------------------
bool FacadeEngineToUI::GetEnttAddedComponents(
    const EntityID id,
    eEnttComponentType* outArr,
    size& numAddedComponents) const
{
    ECS::eComponentType addedComponents[NUM_COMPONENTS];

    if (!pEnttMgr_->GetAddedComponentsByEntt(id, addedComponents, numAddedComponents))
        return false;

    for (index i = 0; i < numAddedComponents; ++i)
        outArr[i] = eEnttComponentType(addedComponents[i]);
...
bool FacadeEngineToUI::AddTransformComponent(
    ...
    const float scale)
{
    pEnttMgr_->AddTransformComponent(id, { p.x, p.y, p.z }, { d.x, d.y, d.z }, scale);
    return true;
}

//---------------------------------------------------------

bool FacadeEngineToUI::AddModelComponent(const EntityID enttId, const ModelID modelId)
{
...
bool FacadeEngineToUI::AddBoundingComponent(
    const EntityID id,
    const int boundType,
    const DirectX::BoundingBox& localBox)
{
    pEnttMgr_->AddBoundingComponent(id, localBox, localBox);
    return true;
}
...
{
    return pEnttMgr_->nameSys_.GetIdByName(name);
}
...
{
    return pEnttMgr_->nameSys_.GetNameById(id);
}
...
{
    outMat = pEnttMgr_->transformSys_.GetWorld(id);
    return true;
}
...
{
    const XMFLOAT3 pos = pEnttMgr_->transformSys_.GetPosition(id);
    return Vec3(pos.x, pos.y, pos.z);
}

Vec3 FacadeEngineToUI::GetEnttDirection(const EntityID id) const
{
    const XMFLOAT3 dir = pEnttMgr_->transformSys_.GetDirection(id);
    return Vec3(dir.x, dir.y, dir.z);
}

Vec4 FacadeEngineToUI::GetEnttRotQuat(const EntityID id) const
{
    const XMVECTOR rotQuat = pEnttMgr_->transformSys_.GetRotationQuat(id);
    return Vec4(
...
{
    return pEnttMgr_->transformSys_.GetScale(id);
}
...
{
    pEnttMgr_->PushEvent(ECS::EventTranslate(id, pos.x, pos.y, pos.z));
    return true;
}

bool FacadeEngineToUI::SetEnttDirection(const EntityID id, const Vec3& dir)
{
    return pEnttMgr_->transformSys_.SetDirection(id, { dir.x, dir.y, dir.z });
}

bool FacadeEngineToUI::SetEnttScale(const EntityID id, const float scale)
{
    return pEnttMgr_->transformSys_.SetScale(id, scale);
}

bool FacadeEngineToUI::RotateEnttByQuat(const EntityID id, const Vec4& q)
{
    return pEnttMgr_->transformSys_.RotateLocalSpaceByQuat(id, { q.x, q.y, q.z, q.w });
}
...
{
    if (!pEnttMgr_->lightSys_.IsLightSource(id))
    {
        lightType = -1;
        return false;
    }

    lightType = pEnttMgr_->lightSys_.GetLightType(id);
    return true;
}
...
{
    ECS::AnimationSystem& animSys        = pEnttMgr_->animationSys_;
...
{
    return pEnttMgr_->animationSys_.GetSkeletonId(enttId);
}
...
{
    return pEnttMgr_->animationSys_.GetAnimationId(id);
}
...
{
    return pEnttMgr_->animationSys_.GetCurrAnimTime(id);
}
...
{
    return pEnttMgr_->animationSys_.GetEndAnimTime(id);
}
    ...
    ColorRGBA& specular)
{
    ...
    if (!pEnttMgr_->lightSys_.GetDirectedLightData(id, data))
    {
    ...
    bool& isActive)
{
    ...
    if (!pEnttMgr_->lightSys_.GetPointLightData(id, data))
    {
    ...
    float& spotExponent)
{
    ...
    if (!pEnttMgr_->lightSys_.GetSpotLightData(id, data))
    {
...
{
    return pEnttMgr_->lightSys_.SetLightIsActive(id, state);
}
...
{
    return pEnttMgr_->lightSys_.IsLightActive(id);
}
...
{
    return pEnttMgr_->lightSys_.SetDirLightProp(id, ECS::LightProp::AMBIENT, rgba.ToFloat4());
}

bool FacadeEngineToUI::SetDirectedLightDiffuse(const EntityID id, const ColorRGBA& rgba)
{
    return pEnttMgr_->lightSys_.SetDirLightProp(id, ECS::LightProp::DIFFUSE, rgba.ToFloat4());
}

bool FacadeEngineToUI::SetDirectedLightSpecular(const EntityID id, const ColorRGBA& rgba)
{
    return pEnttMgr_->lightSys_.SetDirLightProp(id, ECS::LightProp::SPECULAR, rgba.ToFloat4());
}

bool FacadeEngineToUI::SetDirectedLightDirection(const EntityID id, const Vec3& dir)
{
    return pEnttMgr_->lightSys_.SetDirLightProp(id, ECS::LightProp::DIRECTION, XMFLOAT3(dir.x, dir.y, dir.z));
}
...
{
    return pEnttMgr_->lightSys_.GetDirLightProp(id, ECS::LightProp::AMBIENT);
}

ColorRGBA FacadeEngineToUI::GetDirectedLightDiffuse(const EntityID id) const
{
    return pEnttMgr_->lightSys_.GetDirLightProp(id, ECS::LightProp::DIFFUSE);
}

ColorRGBA FacadeEngineToUI::GetDirectedLightSpecular(const EntityID id) const
{
    return pEnttMgr_->lightSys_.GetDirLightProp(id, ECS::LightProp::SPECULAR);
}

Vec3 FacadeEngineToUI::GetDirectedLightDirection(const EntityID id) const
{
    DirectX::XMFLOAT4 dir = pEnttMgr_->lightSys_.GetDirLightProp(id, ECS::LightProp::DIRECTION);
    return Vec3(dir.x, dir.y, dir.z);
}
...
{
    return pEnttMgr_->lightSys_.SetPointLightProp(id, ECS::LightProp::AMBIENT, color.ToFloat4());
}

bool FacadeEngineToUI::SetPointLightDiffuse(const EntityID id, const ColorRGBA& color)
{
    return pEnttMgr_->lightSys_.SetPointLightProp(id, ECS::LightProp::DIFFUSE, color.ToFloat4());
}

bool FacadeEngineToUI::SetPointLightSpecular(const EntityID id, const ColorRGBA& color)
{
    return pEnttMgr_->lightSys_.SetPointLightProp(id, ECS::LightProp::SPECULAR, color.ToFloat4());
}
...
bool FacadeEngineToUI::SetPointLightPos(const EntityID id, const Vec3& pos)
{
    ...
    if (pEnttMgr_->lightSys_.SetPointLightProp(id, prop, position))
    {
...
{
    return pEnttMgr_->lightSys_.SetPointLightProp(id, ECS::LightProp::RANGE, range);
}
...
bool FacadeEngineToUI::SetPointLightAttenuation(const EntityID id, const Vec3& att)
{
    ...
    return pEnttMgr_->lightSys_.SetPointLightProp(id, prop, attenuation);
}
...
{
    return pEnttMgr_->lightSys_.GetPointLightProp(id, ECS::LightProp::AMBIENT);
}

ColorRGBA FacadeEngineToUI::GetPointLightDiffuse(const EntityID id) const
{
    return pEnttMgr_->lightSys_.GetPointLightProp(id, ECS::LightProp::DIFFUSE);
}

ColorRGBA FacadeEngineToUI::GetPointLightSpecular(const EntityID id) const
{
    return pEnttMgr_->lightSys_.GetPointLightProp(id, ECS::LightProp::SPECULAR);
}
...
Vec3 FacadeEngineToUI::GetPointLightPos(const EntityID id) const
{
    ...
    XMFLOAT4 pos = pEnttMgr_->lightSys_.GetPointLightProp(id, ECS::LightProp::POSITION);
    return Vec3(pos.x, pos.y, pos.z);
}
...
Vec3 FacadeEngineToUI::GetPointLightAttenuation(const EntityID id) const
{
    ...
    XMFLOAT4 att = pEnttMgr_->lightSys_.GetPointLightProp(id, ECS::LightProp::ATTENUATION);
...
float FacadeEngineToUI::GetPointLightRange(const EntityID id) const
{
    ...
    XMFLOAT4 range = pEnttMgr_->lightSys_.GetPointLightProp(id, ECS::LightProp::RANGE);
    return range.x;
}
...
{
    return pEnttMgr_->lightSys_.SetSpotLightProp(id, ECS::LightProp::AMBIENT, color.ToFloat4());
}

bool FacadeEngineToUI::SetSpotLightDiffuse(const EntityID id, const ColorRGBA& color)
{
    return pEnttMgr_->lightSys_.SetSpotLightProp(id, ECS::LightProp::DIFFUSE, color.ToFloat4());
}

bool FacadeEngineToUI::SetSpotLightSpecular(const EntityID id, const ColorRGBA& color)
{
    return pEnttMgr_->lightSys_.SetSpotLightProp(id, ECS::LightProp::SPECULAR, color.ToFloat4());
}
...
{
    if (pEnttMgr_->lightSys_.SetSpotLightProp(id, ECS::LightProp::POSITION, XMFLOAT4(pos.x, pos.y, pos.z, 1.0f)))
    {
        return SetEnttPosition(id, pos);
    }
...
{
    return pEnttMgr_->lightSys_.SetSpotLightProp(id, ECS::LightProp::DIRECTION, { dir.x, dir.y, dir.z, 0.0f });
}

bool FacadeEngineToUI::SetSpotLightAttenuation(const EntityID id, const Vec3& att)
{
    return pEnttMgr_->lightSys_.SetSpotLightProp(id, ECS::LightProp::ATTENUATION, XMFLOAT4(att.x, att.y, att.z, 1.0f));
}
...
{
    return pEnttMgr_->lightSys_.SetSpotLightProp(id, ECS::LightProp::RANGE, range);
}

bool FacadeEngineToUI::SetSpotLightSpotExponent(const EntityID id, const float spotExp)
{
    return pEnttMgr_->lightSys_.SetSpotLightProp(id, ECS::LightProp::SPOT_EXP, spotExp);
}
...
{
    return pEnttMgr_->lightSys_.GetSpotLightProp(id, ECS::LightProp::AMBIENT);
}

ColorRGBA FacadeEngineToUI::GetSpotLightDiffuse(const EntityID id) const
{
    return pEnttMgr_->lightSys_.GetSpotLightProp(id, ECS::LightProp::DIFFUSE);
}

ColorRGBA FacadeEngineToUI::GetSpotLightSpecular(const EntityID id) const
{
    return pEnttMgr_->lightSys_.GetSpotLightProp(id, ECS::LightProp::SPECULAR);
}
...
{
    // attenuation values are stored in x,y,z
    const XMFLOAT4 att = pEnttMgr_->lightSys_.GetSpotLightProp(id, ECS::LightProp::ATTENUATION);
    return Vec3(att.x, att.y, att.z);
}
...
float FacadeEngineToUI::GetSpotLightRange(const EntityID id) const
{
    ...
    return pEnttMgr_->lightSys_.GetSpotLightProp(id, ECS::LightProp::RANGE).x;
}
...
float FacadeEngineToUI::GetSpotLightSpotExponent(const EntityID id) const
{
    ...
    return pEnttMgr_->lightSys_.GetSpotLightProp(id, ECS::LightProp::SPOT_EXP).x;
}
    ...
    Vec3& offset)
{
    ...
    const XMFLOAT3 pos = pEnttMgr_->transformSys_.GetPosition(skyEnttID);
...
{
    const EntityID id = pEnttMgr_->nameSys_.GetIdByName("sky");
...
bool FacadeEngineToUI::ImportModelFromFile(const char* filepath, const char* modelName) const
{
    ...
    snprintf(targetDir, sizeof(targetDir), "%s/", modelName);

    return exporter.ExportIntoDE3D(
        &g_ModelMgr.GetModelById(modelId),
        targetDir,
        modelName);
}
...
bool FacadeEngineToUI::GetMaterialTexIds(const MaterialID id, TexID* outTexIds) const
{
    if (!outTexIds)
    {
        LogErr(LOG, "output textures arr == null");
        return false;
    }
    ...
    {
        LogErr(LOG, "no material by id: %" PRIu32, id);
        return false;
    }
...
//---------------------------------------------------------
// Desc:  write data of material by id into output structure (outData)
// Ret:   true if we managed to get proper data
//---------------------------------------------------------
...
{
    const Render::RenderStates& states = pRender_->GetRenderStates();

    switch (type)
    {
        case RND_STATES_RASTER:        return states.GetNumRasterStates();
        case RND_STATES_BLEND:         return states.GetNumBlendStates();
        case RND_STATES_DEPTH_STENCIL: return states.GetNumDepthStencilStates();
    }
...
const cvector<RenderStateName>* FacadeEngineToUI::GetRenderStateNames(const eRndStatesGroup type) const
{
    ...
    switch (type)
    {
        case RND_STATES_RASTER:        return &states.GetRasterStatesNames();
        case RND_STATES_BLEND:         return &states.GetBlendStatesNames();
        case RND_STATES_DEPTH_STENCIL: return &states.GetDepthStencilStatesNames();
    }
...
//---------------------------------------------------------
// Desc:  get responsible render's rasterizer state param / params group by input type
//---------------------------------------------------------
Render::eRsParamType GetRasterParamType(const eRasterProp type)
{
    switch (type)
    {
        case UI_RS_FILL:                     return Render::RS_FILL;
        case UI_RS_CULL:                     return Render::RS_CULL;
        case UI_RS_FRONT_CCW:                return Render::RS_FRONT_COUNTER_CLOCKWISE;
        case UI_RS_DEPTH_BIAS:               return Render::RS_DEPTH_BIAS;
        case UI_RS_DEPTH_BIAS_CLAMP:         return Render::RS_DEPTH_BIAS_CLAMP;
        case UI_RS_SLOPE_SCALED_DEPTH_BIAS:  return Render::RS_SLOPE_SCALED_DEPTH_BIAS;
        case UI_RS_DEPTH_CLIP_ENABLE:        return Render::RS_DEPTH_CLIP_ENABLE;
        case UI_RS_SCISSOR_ENABLE:           return Render::RS_SCISSOR_ENABLE;
        case UI_RS_MULTISAMPLE_ENABLE:       return Render::RS_MULTISAMPLE_ENABLE;
        case UI_RS_ANTIALIASED_LINE_ENABLE:  return Render::RS_ANTIALIASED_LINE_ENABLE;
    }

    return Render::eRsParamType(0);
}

//---------------------------------------------------------
// Desc:  get responsible render's blend state param / params group by input type
//---------------------------------------------------------
Render::eBsParamType GetBlendParamType(const eBlendProp type)
{
    switch (type)
    {
        case UI_BLEND_OPERATION:             return Render::BLEND_OPERATION;
        case UI_BLEND_FACTOR:                return Render::BLEND_FACTOR;
        case UI_BLEND_RND_TARGET_WRITE_MASK: return Render::BLEND_RND_TARGET_WRITE_MASK;
        case UI_BLEND_SRC_BLEND:             return Render::BLEND_SRC_BLEND;
        case UI_BLEND_DST_BLEND:             return Render::BLEND_DST_BLEND;
        case UI_BLEND_OP:                    return Render::BLEND_OP;
        case UI_BLEND_SRC_BLEND_ALPHA:       return Render::BLEND_SRC_BLEND_ALPHA;
        case UI_BLEND_DST_BLEND_ALPHA:       return Render::BLEND_DST_BLEND_ALPHA;
        case UI_BLEND_OP_ALPHA:              return Render::BLEND_OP_ALPHA;
        case UI_BLEND_IS_ALPHA_TO_COVERAGE:  return Render::BLEND_IS_ALPHA_TO_COVERAGE;
        case UI_BLEND_IS_INDEPENDENT:        return Render::BLEND_IS_INDEPENDENT;
        case UI_BLEND_IS_ENABLED:            return Render::BLEND_IS_ENABLED;
    }

    return Render::eBsParamType(0);
}

//---------------------------------------------------------
// Desc:  get responsible render's dss param / params group by input type
//---------------------------------------------------------
Render::eDssParamType GetDepthStencilParamType(const eDepthStencilProp type)
{
    switch (type)
    {
        case UI_DSS_STENCIL_OP:         return Render::DSS_STENCIL_OP;
        case UI_DSS_COMPARISON_FUNC:    return Render::DSS_COMPARISON_FUNC;
        case UI_DSS_DEPTH_ENABLED:      return Render::DSS_DEPTH_ENABLED;
        case UI_DSS_DEPTH_WRITE_MASK:   return Render::DSS_DEPTH_WRITE_MASK;
        case UI_DSS_DEPTH_FUNC:         return Render::DSS_DEPTH_FUNC;
        case UI_DSS_STENCIL_ENABLED:    return Render::DSS_STENCIL_ENABLED;

        // -- front face
        case UI_DSS_FRONT_FACE_STENCIL_FAIL_OP:
            return Render::DSS_FRONT_FACE_STENCIL_FAIL_OP;

        case UI_DSS_FRONT_FACE_STENCIL_DEPTH_FAIL_OP:
            return Render::DSS_FRONT_FACE_STENCIL_DEPTH_FAIL_OP;

        case UI_DSS_FRONT_FACE_STENCIL_PASS_OP:
            return Render::DSS_FRONT_FACE_STENCIL_PASS_OP;

        case UI_DSS_FRONT_FACE_STENCIL_FUNC:
            return Render::DSS_FRONT_FACE_STENCIL_FUNC;

        // -- back face
        case UI_DSS_BACK_FACE_STENCIL_FAIL_OP:
            return Render::DSS_BACK_FACE_STENCIL_FAIL_OP;

        case UI_DSS_BACK_FACE_STENCIL_DEPTH_FAIL_OP:
            return Render::DSS_BACK_FACE_STENCIL_DEPTH_FAIL_OP;

        case UI_DSS_BACK_FACE_STENCIL_PASS_OP:
            return Render::DSS_BACK_FACE_STENCIL_PASS_OP;

        case UI_DSS_BACK_FACE_STENCIL_FUNC:
            return Render::DSS_BACK_FACE_STENCIL_FUNC;
    }

    return Render::eDssParamType(0);
}
...
//        parameter of rasterizer state (RS)
//        (for instance:  a number of all possible cull modes)
//---------------------------------------------------------
int FacadeEngineToUI::GetNumRsParams(const eRasterProp type) const
{
    const Render::RenderStates& rndStates = pRender_->GetRenderStates();
    const Render::eRsParamType  paramType = GetRasterParamType(type);

    return rndStates.GetNumRsParams(paramType);
}

//---------------------------------------------------------
// Desc:  return an array of properties names for some specific
//        parameter of rasterizer state (RS)
//        (for instance:  names of all possible cull modes)
//---------------------------------------------------------
const char** FacadeEngineToUI::GetRsParamsNames(const eRasterProp type) const
{
    const Render::RenderStates& rndStates = pRender_->GetRenderStates();
    const Render::eRsParamType  paramType = GetRasterParamType(type);

    return rndStates.GetArrRsParamsNames(paramType);
}

//---------------------------------------------------------
// Desc:  get string representation of current value of rasterizer state's (RS) property
//---------------------------------------------------------
const char* FacadeEngineToUI::GetRsParamStr(const RsID id, const eRasterProp type) const
{
    const Render::RenderStates& rndStates = pRender_->GetRenderStates();
    const Render::eRsParamType  paramType = GetRasterParamType(type);

    return rndStates.GetRsParamStr(id, paramType);
}

//---------------------------------------------------------
// Desc:  get current value (integer) of rasterizer state's (RS) property
//---------------------------------------------------------
int FacadeEngineToUI::GetRsParamInt(const RsID id, const eRasterProp type) const
{
    const Render::RenderStates& rndStates = pRender_->GetRenderStates();
    const Render::eRsParamType  paramType = GetRasterParamType(type);

    return rndStates.GetRsParamInt(id, paramType);
}

//---------------------------------------------------------
// Desc:  get current value (float) of rasterizer state's (RS) property
//---------------------------------------------------------
float FacadeEngineToUI::GetRsParamFloat(const RsID id, const eRasterProp type) const
{
    const Render::RenderStates& rndStates = pRender_->GetRenderStates();
    const Render::eRsParamType  paramType = GetRasterParamType(type);

    return rndStates.GetRsParamFloat(id, paramType);
}

//---------------------------------------------------------
// Desc:  get current value (boolean) of rasterizer state's (RS) property
//---------------------------------------------------------
bool FacadeEngineToUI::GetRsParamBool(const RsID id, const eRasterProp type) const
{
    const Render::RenderStates& rndStates = pRender_->GetRenderStates();
    const Render::eRsParamType  paramType = GetRasterParamType(type);

    return rndStates.GetRsParamBool(id, paramType);
}


//**********************************************************************************
// get info specific to blending states
//**********************************************************************************
...
{
    const Render::RenderStates& rndStates = pRender_->GetRenderStates();
    const Render::eBsParamType  paramType = GetBlendParamType(type);
...
{
    const Render::RenderStates& rndStates = pRender_->GetRenderStates();
    const Render::eDssParamType paramType = GetDepthStencilParamType(type);
...
//**********************************************************************************
// Update the custom rasterizer / blending / depth-stencil state
//**********************************************************************************

//---------------------------------------------------------
// Desc:  update a rasterizer state's param with input boolean-value
//---------------------------------------------------------
void FacadeEngineToUI::UpdateCustomRsParam(const eRasterProp type, const bool value) const
{
    const Render::eRsParamType paramType = GetRasterParamType(type);
    Render::RenderStates&      rndStates = pRender_->GetRenderStates();

    rndStates.UpdateCustomRsParam(paramType, value);
}

//---------------------------------------------------------
// Desc:  update a rasterizer state's param with input integer-value 
//---------------------------------------------------------
void FacadeEngineToUI::UpdateCustomRsParam(const eRasterProp type, const int value) const
{
    const Render::eRsParamType paramType = GetRasterParamType(type);
    Render::RenderStates& rndStates = pRender_->GetRenderStates();
...
//---------------------------------------------------------
// Desc:  update a rasterizer state's param with input float-value
//---------------------------------------------------------
void FacadeEngineToUI::UpdateCustomRsParam(const eRasterProp type, const float value) const
{
    const Render::eRsParamType paramType = GetRasterParamType(type);
    Render::RenderStates&      rndStates = pRender_->GetRenderStates();
...
//---------------------------------------------------------
// Desc:  update a rasterizer state's param with input string-value
//---------------------------------------------------------
void FacadeEngineToUI::UpdateCustomRsParam(const eRasterProp type, const char* value) const
{
    const Render::eRsParamType paramType = GetRasterParamType(type);
    Render::RenderStates&      rndStates = pRender_->GetRenderStates();
...
//---------------------------------------------------------
// Desc:  update a blending state's param with input boolean-value
//---------------------------------------------------------
void FacadeEngineToUI::UpdateCustomBsParam(const eBlendProp type, const bool value) const
{
    const Render::eBsParamType paramType = GetBlendParamType(type);
    Render::RenderStates&      rndStates = pRender_->GetRenderStates();
...
//---------------------------------------------------------
// Desc:  update a blending state's param with input string-value
//---------------------------------------------------------
...
//---------------------------------------------------------
// Desc:  update a depth-stencil state's param with input boolean-value
//---------------------------------------------------------
void FacadeEngineToUI::UpdateCustomDssParam(const eDepthStencilProp type, const bool value) const
{
    const Render::eDssParamType paramType = GetDepthStencilParamType(type);
    Render::RenderStates&       rndStates = pRender_->GetRenderStates();
...
//---------------------------------------------------------
// Desc:  update a depth-stencil state's param with input string-value
//---------------------------------------------------------
void FacadeEngineToUI::UpdateCustomDssParam(
    const eDepthStencilProp type,
    const char* value) const
{
    if (StrHelper::IsEmpty(value))
    {
...
//**********************************************************************************
// Get/set material's data related to render states
//**********************************************************************************

//---------------------------------------------------------
// Desc:  get id of material's current render state 
//---------------------------------------------------------
uint FacadeEngineToUI::GetMaterialRenderStateId(
    const MaterialID id,
    const eRndStatesGroup type) const
{
    ...
    LogErr(LOG, "unknown render state group: %d    for material (id: %d, name: %s)", type, (int)mat.id, mat.name);
    return 0;
}
    ...
    size numTexTypes) const
{
    if (!texIds)
    {
        LogErr(LOG, "texture IDs arr == null");
        return false;
    }
    if (!outTexViews)
    {
        LogErr(LOG, "output texture views arr == null");
        return false;
    }
    if (numTexTypes != NUM_TEXTURE_TYPES)
    {
        LogErr(LOG, "input number of texture types must == %zu", NUM_TEXTURE_TYPES);
        return false;
    }
...
bool FacadeEngineToUI::SetEnttMaterial(
    ...
    const MaterialID matId)
{
    pEnttMgr_->materialSys_.SetMaterial(enttId, subsetId, matId);
    return true;
}
...
ID3D11ShaderResourceView* FacadeEngineToUI::GetModelFrameBufView() const
{
    ...
    if (!pSRV)
        LogErr(LOG, "can't get a shader resource view of a texture surface into which we rendered a model preview");
    ...
    const uint numElems)
{
    ...
    {
        LogErr(LOG, "can't alloc mem for texture atlas");
        return;
    }
...
void FacadeEngineToUI::SaveTexAtlasToFile(const char* filename)
{
    ...
    if (texAtlasImg.pixels == nullptr)
    {
        LogErr(LOG, "can't save a tex atlas into file: %s\n there is no data!!!", filename);
        return;
    }
...
bool FacadeEngineToUI::GetEnttParticleEmitterData(
    ...
    float& friction)
{
    ECS::EmitterData& emitter = pEnttMgr_->particleSys_.GetEmitterData(id);
    ...
    const bool onOff)
{
    ...
    switch (shapeType)
    {
        case R_DBG_SHAPES:
        {
            // we want to turn off rendering...
            if (onOff == false)
            {
                g_DebugDrawMgr.Shutdown();
                g_ModelMgr.ShutdownDebugLinesBuffers();
                return;
            }

            // we want to turn on rendering...
            if (!g_ModelMgr.InitDebugLinesBuffers())
            {
                LogErr(LOG, "can't init buffers for debug lines");
                return;
            }
        ...
        case R_DBG_SHAPES_LINE:         g_DebugDrawMgr.SwitchRenderingType(LINE, onOff);            break;
        case R_DBG_SHAPES_CROSS:        g_DebugDrawMgr.SwitchRenderingType(CROSS, onOff);           break;
        case R_DBG_SHAPES_SPHERE:       g_DebugDrawMgr.SwitchRenderingType(SPHERE, onOff);          break;
             
        case R_DBG_SHAPES_CIRCLE:       g_DebugDrawMgr.SwitchRenderingType(CIRCLE, onOff);          break;
        case R_DBG_SHAPES_AXIS:         g_DebugDrawMgr.SwitchRenderingType(AXIS, onOff);            break;
        case R_DBG_SHAPES_TRIANGLE:     g_DebugDrawMgr.SwitchRenderingType(TRIANGLE, onOff);        break;

        case R_DBG_SHAPES_AABB:         g_DebugDrawMgr.SwitchRenderingType(AABB, onOff);            break;
        case R_DBG_SHAPES_OBB:          g_DebugDrawMgr.SwitchRenderingType(OBB, onOff);             break;
        case R_DBG_SHAPES_FRUSTUM:      g_DebugDrawMgr.SwitchRenderingType(FRUSTUM, onOff);         break;
        case R_DBG_SHAPES_TERRAIN_AABB: g_DebugDrawMgr.SwitchRenderingType(TERRAIN_AABB, onOff);    break;
        case R_DBG_SHAPES_MODELS_WIRE:  g_DebugDrawMgr.SwitchRenderingType(MODEL_WIREFRAME, onOff); break;
