...
struct CameraData
{
    ...
    DirectX::XMMATRIX baseView = DirectX::XMMatrixIdentity();
    DirectX::XMMATRIX view     = DirectX::XMMatrixIdentity();
    DirectX::XMMATRIX invView  = DirectX::XMMatrixIdentity();
    DirectX::XMMATRIX proj     = DirectX::XMMatrixIdentity();   // projection matrix
    DirectX::XMMATRIX ortho    = DirectX::XMMatrixIdentity();   // orthographic matrix
    ...
    DirectX::XMVECTOR right      { 1,0,0 };    // world space right vector of the camera
    //XMVECTOR up         { 0,1,0 };    // camera's up vector
    DirectX::XMVECTOR target{ 0,1,0 };    // world space look_at point (is used when we concentrate camera on some point)
