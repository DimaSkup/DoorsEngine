...
//---------------------------------------------------------
// Desc:   get params for frustum initialization
// Out:    - fov:          horizontal field of view
//         - aspect:       screen aspect ratio (width divided by height)
//         - nearZ, farZ:  distance to near/far plane by Z-axis
// Ret:    true if there is any camera by input id
//---------------------------------------------------------
bool CameraSystem::GetFrustumInitParams(
    ...
    float& farZ) const
{
    if (HasEntity(camId))
    {
        const CameraData& data = GetCameraData(camId);

        aspect = data.aspectRatio;
        nearZ  = data.nearZ;
        farZ   = data.farZ;
        fov    = 2.0f * atanf(0.5f * aspect * data.nearWndHeight / nearZ);

        return true;
    }
    else
    {
        LogErr(LOG, "there is no camera entity by id: %" PRIu32, camId);
        fov = aspect = nearZ = farZ = 0;
        return false;
    }
}
