    ...
    const DirectX::XMMATRIX& WVO)                 // world * base_view * ortho
{
    ...
    {
        bool result = false;
        const std::string pathToDir = "shaders/";

        result = shadersContainer.colorShader_.Initialize(pDevice, pContext, pathToDir);
        Assert::True(result, "can't initialize the color shader class");

        result = shadersContainer.textureShader_.Initialize(pDevice, pContext, pathToDir);
        Assert::True(result, "can't initialize the texture shader class");

        result = shadersContainer.lightShader_.Initialize(pDevice, pathToDir);
        Assert::True(result, "can't initialize the light shader class");


        result = shadersContainer.fontShader_.Initialize(pDevice, pContext, WVO, pathToDir);
        Assert::True(result, "can't initialize the font shader class");

        result = shadersContainer.debugShader_.Initialize(pDevice, pContext, pathToDir);
        Assert::True(result, "can't initialize the debug shader");

        result = shadersContainer.skyDomeShader_.Initialize(pDevice, pContext, pathToDir);
