...
static const char* s_FillModesNames[NUM_FILL_MODES] = { "Solid", "Wireframe" };
static const char* s_CullModesNames[NUM_CULL_MODES] = { "Back", "Front", "None" };

static const char* s_WindingOrdersNames[NUM_WINDING_ORDERS] =
{
    "Front clockwise",
    "Front counter clockwise"
};

static const char* s_BlendStatesNames[NUM_BLEND_STATES] =
{
...
static const char* s_DepthStencilStatesNames[NUM_DEPTH_STENCIL_STATES] =
{
    "Depth enabled",
    "Depth disabled",
    "Mark mirror",
...
const char** RenderStates::GetFillModesNames()          const { return s_FillModesNames; }
const char** RenderStates::GetCullModesNames()          const { return s_CullModesNames; }
const char** RenderStates::GetWindingOrdersNames()      const { return s_WindingOrdersNames; }
const char** RenderStates::GetBlendStatesNames()        const { return s_BlendStatesNames; }
const char** RenderStates::GetDepthStencilStatesNames() const { return s_DepthStencilStatesNames; }
...
void RenderStates::SetBS(ID3D11DeviceContext* pContext, const eRenderState state)
{
    ...
    try
    {
        pBS = blendStates_.at(state);
    }
    catch (const std::out_of_range&)
    {
        LogErr(LOG, "there is no blend state (BS) by key: %d", state);
        pContext->OMSetBlendState(nullptr, NULL, 0xFFFFFFFF);           // set to default
        return;
    }
...
void RenderStates::InitAllDepthStencilStates(ID3D11Device* pDevice)
{
    ...
    // depth PRE-PASS
    //
    D3D11_DEPTH_STENCIL_DESC depthPreDesc = {};
    depthPreDesc.DepthEnable    = TRUE;
    depthPreDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;
    depthPreDesc.DepthFunc      = D3D11_COMPARISON_LESS_EQUAL;
    depthPreDesc.StencilEnable = FALSE;
    hr = pDevice->CreateDepthStencilState(&depthPreDesc, &depthStencilStates_[R_DEPTH_PREPASS]);
    CAssert::NotFailed(hr, "can't create a depth stencil state (depth pre-pass)");
