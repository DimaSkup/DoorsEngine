// ================================================================================
// Filename:     d3dclass.h
// Description:  here we initialize all the stuff which is reponsible
//               for work with DirectX; enumerate adapters, execute all
//               the primary initialization of devices, etc.
// Revising:     21.03.22
// ================================================================================
#pragma once

#include "AdapterReader.h"
#include "r_states.h"
#include <d3dcommon.h>

namespace Render
{

enum AntiAliasingType : uint8
{
    AA_TYPE_NO_AA,
    AA_TYPE_FXAA,
    AA_TYPE_MSAA,

    NUM_AA_TYPES
};

//---------------------------------------------------------

class D3DClass
{
public:
    D3DClass();
    ~D3DClass();

    // restrict a copying of this class instance
    D3DClass(const D3DClass& obj) = delete;
    D3DClass& operator=(const D3DClass& obj) = delete;
        

    bool Initialize(
        HWND hwnd, 
        const bool vsync, 
        const bool fullScreen, 
        const bool enable4xMSAA,
        const float screenNear, 
        const float screenDepth);

    void SetupViewportParams(
        const float width,
        const float height,
        const float maxDepth,
        const float minDepth,
        const float topLeftX,
        const float topLeftY);

    void Shutdown();

    // execute some operations before each frame and after each frame
    void BeginScene();
    void EndScene();
    

    void GetDeviceAndContext(ID3D11Device*& pDevice, ID3D11DeviceContext*& pContext);
    void GetVideoCardInfo(char* outCardName, const int maxCardNameSize, int& outMemorySize);

    //
    // inline getters
    //
    inline static D3DClass*          Get()                       { return pInstance_; }
    inline ID3D11Device*             GetDevice()           const { return pDevice_; }
    inline ID3D11DeviceContext*      GetDeviceContext()    const { return pContext_; }
    inline ID3D11DepthStencilView*   GetDepthStencilView() const { return pDepthStencilView_;}
    inline ID3D11RenderTargetView*   GetRenderTargetView() const { return pSwapChainRTV_; }
    inline ID3D11ShaderResourceView* GetDepthSRV()         const { return pDepthSRV_; }
    inline ID3D11ShaderResourceView* GetResolvedDepthSRV() const { return pResolvedDepthSRV_; }
    inline DXGI_FORMAT               GetBackBufferFormat() const { return backBufFormat_; }

    inline int                       GetWindowWidth()      const { return wndWidth_; }
    inline int                       GetWindowHeight()     const { return wndHeight_; }
    inline float                     GetAspectRatio()      const { return (float)wndWidth_ / (float)wndHeight_; }
    inline float                     GetNearZ()            const { return nearZ_; }
    inline float                     GetFarZ()             const { return farZ_; }

    inline bool                      IsEnabledFXAA()       const { return currAntiAliasing_ == AA_TYPE_FXAA; }
    inline bool                      IsEnabled4xMSAA()     const { return enable4xMsaa_; }

    void SetAntiAliasingType(const uint8 aaType);
    inline AntiAliasingType GetAntiAliasingType()          const { return currAntiAliasing_; }
 

    inline RenderStates& GetRenderStates()                       { return renderStates_; }

    // bind/unbind depth buffer is used for depth fullscreen visualization
    inline void BindDepthBuffer()                                { pContext_->OMSetRenderTargets(1, &pSwapChainRTV_, pDepthStencilView_); }
    inline void UnbindDepthBuffer()                              { pContext_->OMSetRenderTargets(1, &pSwapChainRTV_, nullptr); }


    // set default render target/viewport
    inline void ResetBackBufferRenderTarget()                    { pContext_->OMSetRenderTargets(1, &pSwapChainRTV_, pDepthStencilView_); }
    inline void ResetViewport()                                  { pContext_->RSSetViewports(1, &viewport_); }

    // turning the Z buffer on and off when rendering 2D images
    void TurnOnRSfor2Drendering(void);
    void TurnZBufferOn(void);
    void TurnZBufferOff(void);


    // fullscreen/windowed stuff
    bool ToggleFullscreen(HWND hwnd, bool isFullscreen);
    inline SIZE GetWindowedWndSize()                       const { return { windowedModeWidth_, windowedModeHeight_ }; }
    inline SIZE GetFullscreenWndSize()                     const { return { fullScreenWndWidth_, fullScreenWndHeight_ }; }

    // handler for the window resizing
    bool ResizeSwapChain(HWND hwnd, const UINT width, const UINT height);

private:

    void InitDirectX(
        HWND hwnd, 
        const UINT windowWidth,
        const UINT windowHeight, 
        const float nearZ, 
        const float farZ);

    void InitDevice();
    void InitSwapChain(HWND hwnd, const int width, const int height);
    void InitRenderTargetView(const UINT width, const UINT height);

    // initialize depth stencil parts
    void InitDepthStencil      (const UINT width, const UINT height);
    void InitDepthStencilTexBuf(const UINT width, const UINT height, const UINT mipLevels);

    void InitDepthStencilView();

    void InitDepthStencilSRV     (const UINT mipLevels);
    void InitViewport            (const UINT width, const UINT height);
    void InitDepthResolvedTexture(const UINT width, const UINT height, const UINT mipLevels);
    void InitDepthPrepassTargets (const UINT width, const UINT height, const UINT mipLevels);

    void ReleaseTargets();

    void ReleasePostEffectsStuff();
    void ReleaseMSAATargets();
    void ReleaseDepthPrepassTargets();
    
public:
    static D3DClass* pInstance_;

    IDXGISwapChain*			  pSwapChain_          = nullptr;    
    ID3D11Device*			  pDevice_             = nullptr;    
    ID3D11DeviceContext*	  pContext_            = nullptr;    
    ID3D11RenderTargetView*   pSwapChainRTV_       = nullptr;    // where we are going to render our buffers
    D3D11_VIEWPORT            viewport_{ 0 };

    // MSAA targets (for post process)
    ID3D11Texture2D*          pMSAAColorTex_       = nullptr;
    ID3D11RenderTargetView*   pMSAARTV_            = nullptr;
    ID3D11ShaderResourceView* pMSAASRV_            = nullptr;

    // resolved targets (non-MSAA for post process)
    ID3D11Texture2D*          postFxsPassTex_[2]{ nullptr };
    ID3D11RenderTargetView*   postFxsPassRTV_[2]{ nullptr };
    ID3D11ShaderResourceView* postFxsPassSRV_[2]{ nullptr };

    // depth prepass targets (and used for SSAO as well)
    ID3D11Texture2D*          pNormalDepthTex_     = nullptr;
    ID3D11RenderTargetView*   pNormalDepthRTV_     = nullptr;
    ID3D11ShaderResourceView* pNormalDepthSRV_     = nullptr;

    // depth stencil stuff
    ID3D11Texture2D*		  pDepthStencilBuffer_ = nullptr;
    ID3D11DepthStencilView*	  pDepthStencilView_   = nullptr;
    ID3D11ShaderResourceView* pDepthSRV_           = nullptr;

    // for depth visualization
    ID3D11Texture2D*          pResolvedDepthTex_   = nullptr;
    ID3D11ShaderResourceView* pResolvedDepthSRV_   = nullptr;
    

    AdapterReader             adaptersReader_;
    RenderStates              renderStates_;

    AntiAliasingType          currAntiAliasing_ = AA_TYPE_NO_AA;

    HWND                      hwnd_;

    DXGI_FORMAT backBufFormat_  = DXGI_FORMAT_R8G8B8A8_UNORM;
    int   wndWidth_             = 800;       // current window width
    int   wndHeight_            = 600;       // current window height
    int   windowedModeWidth_    = 800;       // current wnd width for the windowed mode (we will use it when turn back to the windowed mode)
    int   windowedModeHeight_   = 600;       // current wnd height for the windowed mode
    int   fullScreenWndWidth_   = 800;       // current wnd width for the full screen mode
    int   fullScreenWndHeight_  = 600;       // current wnd height for the full screen mode
    float nearZ_                = 0.0f;
    float farZ_                 = 100.0f;
    
    bool enableAltEnter_        = true;
    bool vsyncEnabled_          = false;
    bool fullScreen_            = false;
    bool enable4xMsaa_          = false;   // use 4X MSAA?
    UINT m4xMsaaQuality_        = 0;       // 4X MSAA quality level
    UINT displayAdapterIndex_   = 0;       // set adapter idx (if there is any discrete graphics adapter we use this discrete adapter as primary)
};


//==================================================================================
// Global pointers of DX11 device and device context
//==================================================================================
extern ID3D11Device*        g_pDevice;
extern ID3D11DeviceContext* g_pContext;

} // namespace Core
