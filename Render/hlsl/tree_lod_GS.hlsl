    ...
    inout TriangleStream<GS_OUT> triStream)
{
	...
	const float stageL = gin[0].stage.x;
	const float stageR = gin[0].stage.y;
	...
	tex0[2].x = stageL * inv + inv;
	tex0[3].x = stageL * inv + inv;
	
	
	tex1[0].x = stageR * inv;
	tex1[1].x = stageR * inv;
	
	tex1[2].x = stageR * inv + inv;
	tex1[3].x = stageR * inv + inv;
    ...
    for (int i = 0; i < 4; ++i)
    {
		...
		gout.worldInvTranspose 	= gin[0].worldInvTranspose;
