...
bool Image::Create(const uint width, const uint height, const uint bpp)
{
    ...
    // allocate memory
    const int numBytes = width * height * (bpp / 8);

    pixels_ = NEW uint8[numBytes];
    if (!pixels_)
    {
        LogErr(LOG, "can't allocate memory for an empty image");
        return false;
    }

    // fill in the pixels buffer with zeros
...
bool Image::LoadData(const char* filename)
{
    ...
    try
    {
        ...
        isLoaded_ = true;
        LogMsg(LOG, "File is loaded correctly: %s", filename);
        return true;
    }
...
bool Image::Save(const char* filename)
{
    ...
    {
        LogErr(LOG, "can't save: image is empty or corrupted (for file: %s)", filename);
        return false;
    }
    ...
    {
        LogErr(LOG, "can't save image into file: unsupported format (%s)", extension);
        return false;
    }

    LogMsg(LOG, "The height map was saved successfully: %s", filename);
    return true;
}
...
bool Image::LoadRgbBMP(const char* filename)
{
    ...
    try
    {
        ...
        {
            throw EngineException("can't create an empty space for bmp image");
        }
...
bool Image::LoadGrayscaleBMP(const char* filename)
{
    ...
    try
    {
        ...
        tmpImageBuf = NEW uint8[infoHeader.sizeImage];
        if (!tmpImageBuf)
        {
            throw EngineException("can't allocate memory for image temp buffer");
        }

        // fill in the temp buffer with zeros
        memset(tmpImageBuf, 0, infoHeader.sizeImage);

        // read in the bitmap image data
        retCode = fread(tmpImageBuf, 1, infoHeader.sizeImage, pFile);
        ...
        for (int j = 0; j < height; ++j)
        {
            ...
            for (int i = 0; i < width; ++i)
            {
                ...
                pixels_[rowIdx + i] = tmpImageBuf[k];
...
// Args:   - filename:  a path to image file
//--------------------------------------------------------------
