...
//---------------------------------------------------------------------------------------
void CalcFog(
    const float4 skyColor,
    const float3 fogColor,
    const float distToEye,
    const float fogStart,
    const float fogRange,
    inout float4 pixelColor)
{
    // blend sky pixel color with fixed fog color
    const float4 finalFogCol = skyColor * float4(fogColor, 1.0);

    const float fogLerp = saturate((distToEye - fogStart) / fogRange);

    // blend the fog color and the lit color
    pixelColor = lerp(pixelColor, finalFogCol, fogLerp);
}
