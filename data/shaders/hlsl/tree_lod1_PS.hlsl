...
float4 PS(PS_IN pin) : SV_TARGET
{
	...
	const float2 fadeRange = float2(40, 50);
	...
	//return float4(blend, blend, blend, 1.0);
	...
	{
		diffTex = gTextures[1].Sample(gSamLinearWrap, pin.tex1);
		
		// execute alpha clipping
		clip(diffTex.a - 0.7f);
		
		normalMap = gTextures[6].Sample(gSamLinearWrap, pin.tex1).rgb;
	}
